...
# Direct Credential Injection (Bypasses Secrets Glitch)
...
MASTER_PASSWORD = "YOUR_PASSWORD" # Change this to your chosen password
...
class TitanV5:
    ...
    def check_funding_shield():
        # Logic Gate: Call Shield & Put Shield
        return True 
...
# 7. VISUAL VALIDATOR: AUTO-PHOTO & EXECUTION
    ...
    if col_a.button("🚀 AUTO-BUY (10x | 50% TP)", use_container_width=True):
        if TitanV5.check_funding_shield():
            st.session_state.order_history.append({"Time": datetime.datetime.now().strftime("%H:%M"), "Side": "BUY", "Target": "50%", "Status": "Active"})
            TitanV5.log_signal("Pink Alert Triggered", True, "BUY", 65000)
            st.success("Trade Executed!")
    
    if col_b.button("📉 AUTO-SELL (10x | 10% TP)", use_container_width=True):
        st.session_state.order_history.append({"Time": datetime.datetime.now().strftime("%H:%M"), "Side": "SELL", "Target": "10%", "Status": "Active"})
